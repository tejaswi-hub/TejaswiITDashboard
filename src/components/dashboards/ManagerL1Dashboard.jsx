...
const ManagerL1Dashboard = () => {
  ...
  // Filter tickets assigned to L1 team
  const teamTickets = useMemo(() => {
    return tickets.filter(ticket => ticket.assignedL1 === user.name);
  }, [tickets, user.name]);
  ...
  const kpis = useMemo(() => {
    ...
    const avgClosureTime = "1.8 days"; // This would be calculated from actual data
    ...
    return [
      ...
      },
      {
        ...
        value: openTickets,
        icon: Clock,
        trend: openTickets > 5 ? 'up' : 'down',
        trendValue: openTickets > 5 ? 'High workload' : 'Manageable'
      },
      {
